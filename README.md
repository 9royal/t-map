# T map v1.05.1 — 多地圖平台臺灣互動修正版

T map v1.05.1 延續 v1.05 多地圖平台，並把 v1.04.6 已驗證的放大鏡準星接觸邏輯重新整合。原本以臺灣為核心的單一地圖程式，改造成可持續擴充的多地圖平台。這一版的重點不是一次塞入所有新圖資，而是先建立共用首頁、地圖登錄表、共用拼圖資料介面與語音模式設定，並確認既有臺灣模組仍可完整使用。
